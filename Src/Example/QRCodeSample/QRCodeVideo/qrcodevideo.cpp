...
// 	Mat traces(image.size(), CV_8UC3);								// For Debug Visuals
	Mat gray;			// To hold Grayscale Image
	Mat bin;
	Mat edges;			// To hold Grayscale Image
	...
	while (1)
	{
		...
		gray = Mat(image.size(), CV_MAKETYPE(image.depth(), 1));
		bin = Mat(image.size(), CV_MAKETYPE(image.depth(), 1));
		edges = Mat(image.size(), CV_MAKETYPE(image.depth(), 1));
		traces = Mat(image.size(), CV_8UC3);
		break;
	}
// 	if (image.empty()){
...
// 	}
	...
	while (key != 'q')				// While loop to query for Image Input frame
	{
		...
		//threshold(gray, bin, 60, 255, cv::THRESH_BINARY);
		Canny(gray, edges, 100, 200, 3);		// Apply Canny edge detection on the gray image
		//Canny(bin, edges, 10, 200, 3);		// Apply Canny edge detection on the gray image
		//imshow("gray", bin);
		imshow("edges", edges);
